...
or font families under the `extend` section. The application now uses the **Poppins** font via the `fontFamily.sans` configuration. Reusable custom
classes should be
placed inside the `@layer components` block in `src/index.css`.
Placeholder SVG images embed `font-family="Poppins, sans-serif"` so fallback text uses a generic sans-serif font.
...
The application loads weeks 1–41. When adding or removing weeks, update the
`TOTAL_WEEKS` constant in `src/contexts/ContentProvider.jsx`. Completing the
final week keeps progress locked at week 41 and logs **"Course Finished!"** in
the browser console.

Weeks are grouped into four terms. Term 1 and Term 2 contain ten weeks each.
Term 3 now runs from week 21 through week 30 (ten weeks total), and Term 4 has
eleven weeks. The helper
