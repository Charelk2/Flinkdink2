...
dog breed names are automatically translated (case-insensitively) to their English equivalents so Unsplash can
find matching photos. For example `/api/photos?query=Jagwindhond` searches Unsplash for **greyhound**
while `Engelse Patryshond` searches for **English pointer**.
