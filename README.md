...
The red dots displayed on math slides still use Tailwind's `inline-block` utility, but their width and height are now set inline to avoid them collapsing if the build misses the `w-4`/`h-4` classes.
