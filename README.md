...
The `src/utils/encyclopediaImages.js` module uses Vite's `import.meta.globEager` to
load every file in that directory and build an object keyed by slug. React
components can look up a matching image by its slug without explicit import
statements.
